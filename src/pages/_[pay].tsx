...
const Pay: React.FC<PayProps> = () => {
  ...
  const [values] = useState({
    ...
    pageDescription: query.get("pageDescription") !== null ? query.get("pageDescription") : '',
    customerFullName: query.get("customerFullName") !== null ? query.get("customerFullName") : '',
    customerPhoneNumber: query.get("customerPhoneNumber") !== null ? query.get("customerPhoneNumber") : '',
    customerEmailAddress: query.get("customerEmailAddress") !== null ? query.get("customerEmailAddress") : '',
    transactionReference: query.get("transactionReference") !== null ? query.get("transactionReference") : '',
    currency: query.get("currency") !== null ? query.get("currency") : '',
    amount: query.get("amount") !== null ? query.get("amount") : '',
    redirectURL: query.get("redirectURL") !== null ? query.get("redirectURL") : '',
    processId: processId
  });
  ...
  const onSubmit = (v: Merchant) => {
    ...
    v.pageDescription = (values.pageDescription) !== null ? values.pageDescription : '';
    v.pageLogo = (values.pageLogo) !== null ? values.pageLogo : '';
    v.redirectURL = (values.redirectURL) !== null ? values.redirectURL : '';
    v.transactionReference = (values.transactionReference) !== null ? values.transactionReference : '';
    dispatch(paymentRequest(v));
  }; 

  useEffect(() => {
    const { isPaymentFailure, isPaymentSuccess, isSubmitting, orderResponse, error } = home;
  ...
  return (
    <Layout>
      <Content style={{ background: "#fff" }}>
        ...
        >
          <Col span={6}>
            <div className="pay-details">
              <Image width={100} src={values.pageLogo !== null ? values.pageLogo : "https://via.placeholder.com/150"} />
        ...
        <PayForm values={values} onSubmit={onSubmit} isSubmit={isSubmit} error={errorData} />
        <Row
          style={{
            display: "flex",
            flexWrap: "wrap",
